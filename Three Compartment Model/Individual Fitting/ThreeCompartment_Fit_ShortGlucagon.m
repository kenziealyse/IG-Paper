...
% Fit the data     
residual = @(params) myres_shortGlucagon(params, pig_number);
...
insulin_data = pig_data.insulin_data(pig_number,1:end-1);
insulin_time = pig_time_data.insulin(1:end-1);
init_cond_insulin = insulin_data(1);

glucose_data = pig_data.glucose_data(pig_number,1:end-1);
init_cond_glucose = glucose_data(1);

glucagon_data = pig_data.glucagon_data(pig_number,1:end-1);
