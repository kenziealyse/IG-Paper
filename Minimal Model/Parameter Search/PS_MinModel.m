...
function Fit_Data_Information = PS_MinModel(filename, num_of_params, num_of_sets, scales, param_names)
...
X_values = zeros(length(tspan_to_save), num_of_sets);
...
algorithm = strings([num_of_sets, 1]);
        ...
        parfor (i = 1:num_of_sets)
            ...
            % Fit the data     
            residual = @(init_guess) myres(init_guess);
            ...
            [~,Y] = ode23s(@(t,Y) MinModel_func(t,Y, estimates, @dosing_func), tspan_to_save, init_cond);
    ...
    Fit_Data_Information.Remote_insulin_Values = X_values;
    ...
    Fit_Data_Information.insulin_resid = blood_insulin_resid;
    Fit_Data_Information.glucose_resid = glucose_resid;
