...
function Fit_Data_Information = Min_Model_Param_Search_fits(gamma_vals, ...
    filename_strings, num_of_params, num_of_sets, scales, param_names, ...
    model)
...
for k = 1:length(gamma_vals)
    ...
    waitbar((k -1) /length(gamma_vals),f, sprintf('Working on %d', k));
    
    % Set Parameter Values
    gamma = gamma_vals(k);
    filename = filename_strings{k};
    
    % Pre Allocate Space
    param_values = zeros(num_of_sets, num_of_params);
    glucose_resid = zeros(num_of_sets, 1);
    blood_insulin_resid = zeros(num_of_sets, 1);
    ...
    algorithm = strings([num_of_sets, 1]);
        ...
        parfor (i = 1:num_of_sets)
            ...
            residual = @(params) myres(params, ...
                   tspan, init_cond, insulin_data, glucose_data, glucagon_data, gamma, model, @dosing_func);
            ...
            Glucose_values(:,i) = G;
            Insulin_values(:,i) = I;
            Glucagon_values(:,i) = GL;
            Beta_cell_values(:,i) = B; 

        
        end
        
    waitbar((k) /length(gamma_vals),f, sprintf('Saving data for %d', k));
    ...
    Fit_Data_Information.Gamma_values = gamma_vals;
